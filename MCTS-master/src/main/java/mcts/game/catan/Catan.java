...
public class Catan implements Game, GameStateConstants {
	...
	public static int[][] dataPointsMatrix = new int[4][8];
	public static int[] devCards = new int[4]; //Updated on line 358
	public static int[][] numberList = new int [4][13];
	
	//[x]Player
	//[x]Resource Strength - Only counts once the hex has been rolled on
	//[x]Hand Strength - total resources * variety
	//[x]VP Total - handled in recalcScore()
	//[x]Cities - handled on line 295
	//[x]Dev Cards - line 358
	//[x]Round - takes a snapshot at each round - handled in main
	//[x]Win - writes the winner of the final round. Other rounds should be filled in during data processing
	...
	public void performAction(int[] a, boolean sample) {
        ...
        case A_BUILDSETTLEMENT: 
			...
			if (fsmstate == S_SETTLEMENT2) {
			...
			} else if (fsmstate == S_NORMAL) {	//Build the settlement
				...
				//System.out.println(pl + " built a settlement");
		...
		case A_BUILDCITY:
			...
			//System.out.println(pl + " built a city. Total cities: " + OFS_NCITIES);
			
			dataPointsMatrix[pl][4]++;	//increase the # of cities
        ...
        case A_BUYCARD:
            ...
            //System.out.println(pl + " bought a dev card");
            devCards[pl]++;	//increase hand strength
        	break;
        case A_DEAL_DEVCARD:
            val = a[1];
	...
	public void calcResources(int player) {	//all these if statements should really be another method
		int variety  = 0;
		int total = 0;
		if(state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_WOOD] != 0) {
			variety++;
			total += state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_WOOD];
		};
		if(state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_CLAY] != 0) {
			variety++;
			total += state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_CLAY];
		}
		if(state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_SHEEP] != 0) {
			variety++;
			total += state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_SHEEP];
		}
		if(state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_STONE] != 0) {
			variety++;
			total += state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_STONE];
		}
		if(state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_WHEAT] != 0) {
			variety++;
			total += state[OFS_PLAYERDATA[player] + OFS_RESOURCES + RES_WHEAT];
		}
		dataPointsMatrix[player][2] = total * variety; //TODO: change
		for(int i = 0; i < 4; i++) {
			dataPointsMatrix[i][5] = devCards[i];
		}
	}
	public void calcProductionNum(int player) {
		dataPointsMatrix[player][1] = 0;	//clear the resource strength to reevaluate it
		int score = 0;
		for(int i = 2; i < 13; i++) {
			if(numberList[player][i] > 0) {
				if(i == 2 || i == 12)
					score++;
				else if(i == 3 || i == 11)
					score = score + 2;
		...
		dataPointsMatrix[player][1] = score;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		long d = 0;
		double b = 0;
		int nGames = 10000;		//number of games run 
		Catan game = null;
		//FileWriter writer = new FileWriter("D://extracted.csv", true);
		int totalRounds = 0;
		for(int i =0; i < nGames; i++) {
			...
			game = new Catan(config);
			while(!game.isTerminal()) {
				if(round % 4 == 0) { //a tick is everytime it changes player so we only want every 4
					for (int k = 0; k < 4; k++) {
						dataPointsMatrix[k][6] = (round/4) - 4;
						if(dataPointsMatrix[k][6] > 0) {
							dataPointsMatrix[k][0] = k;
							for(int j = 0; j < 7; j++) {
								//writer.append(String.valueOf(dataPointsMatrix[k][j]) + ",");
							}
							//writer.append("0");
							//writer.append("\n");
						}
			...
			for(int k = 0; k < 4; k++) {
				if(game.getWinner() == k)
					dataPointsMatrix[k][7] = 1;
				else
					dataPointsMatrix[k][7] = 0;
				
			}
			for (int k = 0; k < 4; k++) {
				dataPointsMatrix[k][6] = round/4 + 1;
				for(int j = 0; j < 8; j++) {
					//writer.append(String.valueOf(dataPointsMatrix[k][j]) + ",");
				}
				//writer.append("\n");
			}
			totalRounds += dataPointsMatrix[0][6];
			//Erase the contents of dataPointsMatrix for the next round
			for(int k = 0; k < 4; k++) {
				for(int j = 0; j < 8; j++) {
					dataPointsMatrix[k][j] = 0;
					devCards[k] = 0;
				}
			}
			round = 0;
		...
		System.out.println("Average length: " + totalRounds/10000);
		/*for(int i = 0; i < 4; i++) {
			for(int j = 0; j < 13; j++) {
